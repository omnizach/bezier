...
(function() {
  ...
  d3.bezier = function() {
    ...
    paintPath = function(c) {
      ...
      n0 = c.normal(0);
      n3 = c.normal(1);
      return "M " + (c.p0.x - n0.x * w) + ", " + (c.p0.y - n0.y * w) + " L " + (c.p0.x + n0.x * w) + ", " + (c.p0.y + n0.y * w) + " L " + (c.p3.x + n3.x * w) + ", " + (c.p3.y + n3.y * w) + " L " + (c.p3.x - n3.x * w) + ", " + (c.p3.y - n3.y * w) + " Z";
    };
    drawPath = function(c) {
    ...
    colorScale = d3.scale.linear().domain([0, 1]).interpolate(d3.interpolateHsl).range(['#c00000', '#008000']);
    color = function(c) {
      return colorScale(c.segmentOffset);
    };
    closed = false;
    normalize = 'none';
    width = function() {
    ...
    my = function(selection) {
      return selection.each(function(data) {
        var g, knots, spline;
        knots = d3.zip(data.map(x), data.map(y)).map(function(p) {
          return {
            x: p[0],
            y: p[1]
          };
...
//# sourceMappingURL=bezier.d3.js.map
